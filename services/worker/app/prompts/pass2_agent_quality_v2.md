...
prompt_version: pass2-agent-quality-v2
...
4. CONVERSATION STAGE REACHED — the furthest stage reached:
   ...
   ⚠️ Do NOT stop at `offer_presented` when the conversation continued past
   the offer. The stage keeps advancing:
   - The customer discussed, questioned, pushed back on, or bargained over
     the stated offer (price, contents, dates) → `negotiation`.
   - The agent asked for payment, a deposit, documents to book, or an
     explicit booking commitment → `closing_attempted`, even if the customer
     did not agree.
   - The customer explicitly agreed to buy/book → `deal_closed`.
   `offer_presented` is only correct when the offer was stated and the call
   ended with no substantive discussion of it.
...
A request for a discount before any price has been quoted is NOT a price
objection. Objections 1-3 require the customer to push back on something the
agent actually offered.

⚠️ MISSING an objection silently removes the whole module from the grade —
that is the single most common judging error on this rubric. Scan the call
against these cues before concluding "no objection appeared":

- Price too expensive — after a price was stated: "غالي" / "كثير" / "مرتفع" /
  "ما توقعت كذا" / "الميزانية ما تكفي" / silence-then-hesitation about the
  price. Any pushback on a quoted number counts.
- Cheaper elsewhere — "لقيت أرخص" / "شركة ثانية عرضت" / "في موقع أرخص" /
  any comparison to another provider's offer.
- Need time to think — "أفكر" / "أستشير" / "أرد عليك" / "أكلمك بعدين" /
  "خلني أشوف" — postponing a decision on a live offer IS this objection.
- Service not available — ⚠️ this one does NOT require customer pushback:
  it fires whenever the customer asked for something and the AGENT declared
  it unavailable or refused — "ما عندنا" / "لا والله" / "ما نقدر" / "مش
  متوفر" / "ما نسوي كذا". The module then scores HOW the refusal was
  delivered: professional apology + alternative or referral = full marks,
  apology alone = partial, a bare refusal with no apology and no
  alternative = 0. A bare "لا والله ما عندنا" is a scored 0 — never `null`.
...
- Apologized professionally AND offered an alternative available service = 25 pts
- Apologized professionally but offered no alternative = 15 pts
- Introduced customer to other suitable company services = 25 pts
- Ignored or responded negatively = 0 pts
