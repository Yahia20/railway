<!-- prompt_version: pass1-customer-v3 · schema_version: 1.0
...
- `intent`: the customer's reason for contact, one of:
  ...
  Decision rules — apply in this order, and use the ENUM VALUE EXACTLY:
  - `booking_request` ONLY when the customer expresses intent to book or pay
    NOW ("أبغى أحجز", "اعملوا لي الحجز", "جاهز أدفع"). Asking what packages
    exist or what they cost — even with strong interest — is
    `price_inquiry` or `availability_check`, not a booking.
  - `price_inquiry`: the question centres on cost ("بكم", "كم السعر",
    "عندكم أرخص").
  - `availability_check`: the question centres on whether a SPECIFIC
    service/route/date/visa exists or can be done ("عندكم باكج لبولندا؟",
    "تسوون فيزا للصين؟", "في رحلة يوم الجمعة؟").
  - `support`: about a booking or service that ALREADY exists (paid, booked,
    in progress). If no existing booking is involved, it is not support.
  - `general_info`: a general question answerable without any trip being
    planned (office hours, location, how the company works).
  - `other`: not a customer at all — job seekers, suppliers, partners,
    someone asking to be connected to a specific employee, wrong numbers.
