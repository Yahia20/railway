<!-- prompt_version: pass1-customer-v5 · schema_version: 1.0
     ...
     summary.
     v6: `requests[]` — a conversation can contain more than one thing the
     customer wants, and every earlier version could only carry one. The
     single-value fields above it still describe the PRIMARY request, so
     nothing downstream changes; the array is additive.

     v5: quotes in `real_ask.evidence` and `promises_made_by_agent` are now
     checked in code against the conversation, one uninterrupted span at a
     time, and the verdict is returned in `pass1_validation`; the wording here
     says so. Plus an indirect-negativity note on `objections` and
     `lead_temperature`, from the same dialect-sentiment finding that drove
     pass2 v4. No enum, field or output shape changed. -->
...
- `objections[].kind`: `price_expensive`, `cheaper_elsewhere`, `need_time`,
  ...
  ⚠️ In Gulf and Egyptian Arabic an objection is usually POLITE, INDIRECT or
  joking, and it is still an objection. Record the action the customer is
  taking, not the warmth of the words:
  - "إن شاء الله أشوف وأرد عليك" / "خليني أفكر" after a price → `need_time`.
  - "غالي شوي بس ماشي" → `price_expensive`; the concession does not delete it.
  - "ما شاء الله سعر ممتاز!" said sarcastically after a high quote →
    `price_expensive`.
  ⚠️ Do not infer an objection from non-purchase, silence, or terminal courtesy
  alone. A closing thanks such as "تمام، جزاك الله خير" remains neutral unless
  the customer's words also express deferral, reconsideration, future response,
  unwillingness, comparison, or pushback. Re-read eligible conversations for
  those textual signals, but leave `objections` EMPTY when no trigger is
  present. A polite phrase used mid-conversation while the customer keeps
  engaging is likewise NOT an objection.
  None of this changes the enums, and it cuts both ways for
  `lead_temperature`: a customer who explicitly defers is `warm` or `cold`, not
  `hot` — reading courtesy as enthusiasm is what puts dead leads at the top of
  the follow-up queue. But a bare closing thanks is not a deferral either, and
  must not on its own cool a customer who was otherwise ready to move.
  Keep the customer's literal words in `objections[].quote`: verbatim, one
  uninterrupted span, same rule as `real_ask`.
...
- `real_ask`: did the customer make a REAL inquiry about a tourism product —
  ...
  cannot quote it, the answer is `false`. **Verbatim means ONE UNINTERRUPTED
  SPAN**: copy a short run of characters exactly as it appears, never joining
  two parts of a message, never trimming the middle, never adding "…", never
  tidying spelling or punctuation, and never quoting across an `[[ASR_GAP]]`
  marker. Every quote here and in `promises_made_by_agent` is checked in code
  against the conversation, and one that is not found is reported as
  unverified — the follow-up task built on it is then unsafe to act on. Ten
  words that are really there beat thirty that have been smoothed. This flag
  feeds the sales follow-up
...
- `requests`: EVERY distinct thing the customer asked for, one entry each, in
  the order they were first raised. A customer who asks about a Turkey package
  and then a Schengen visa for his brother made TWO requests, not one — and the
  agency opens one deal per request, so a request you omit is revenue that
  nobody will ever see was missed.

  Rules, all of them load-bearing:

  1. **Entry 1 is the primary request** — the one the top-level `intent`,
     `service`, `trip` and `commercial` fields already describe. When the
     conversation holds exactly one request, `requests` has exactly one entry
     and it repeats what is above it. That repetition is intentional.
  2. **The primary request is the one the customer pursued furthest**: the
     highest `buying_stage` wins; if two are level, the one raised first wins.
     Not the largest budget, and not the last one mentioned.
  3. **Every entry needs a verbatim quote in `evidence`** — the customer's own
     words asking for that specific thing, one uninterrupted span, under the
     same rule as `real_ask.evidence`. Every quote is checked in code against
     the conversation. An entry whose quote is not found is discarded from the
     opportunity count, because an invented request sends a salesperson after
     a customer who never asked for it.
  4. **Split only on a different PRODUCT.** "Istanbul then Trabzon" is one
     package with two destinations, not two requests. "A package, and also a
     visa" is two. When the customer asks the same thing twice, that is one
     entry.
  5. **A request the agent proposed is not a request** unless the customer took
     it up. `requests` is what the CUSTOMER wanted.
  6. If `real_ask.is_real_inquiry` is `false`, `requests` is `[]`. A supplier
     pitching us, a wrong number and a job application contain no requests.
...

{
  ...
  "requests": [
    {
      "service": "package | flight | hotel | cruise | visa | transfer | insurance | other | unknown",
      "service_raw": null,
      "intent": null,
      "destination": null,
      "date_start": null,
      ...
      "budget_amount": null,
      "budget_currency": null,
      "buying_stage": "awareness | consideration | decision | purchased | lost | unknown",
      "evidence": [ { "quote": "verbatim customer words asking for THIS thing", "timestamp": null } ]
    }
  ],
  "special_requests": [],
