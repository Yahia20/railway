...
prompt_version: pass2-agent-quality-v4
revision: v4.1
...
The file name stays pass2_agent_quality_v4.md and PASS2_VERSION stays
`pass2-agent-quality-v4`: the rubric, the criteria, the enums and the JSON
schema are untouched, and compare_day.py's cache key hashes prompt CONTENTS,
so an edit invalidates every cached answer without a version bump. `revision`
above is what distinguishes the two texts in review.
-->
